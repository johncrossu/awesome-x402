...
- [Cosmic Live Data](https://x402.dotrockets.com) - Real-time geophysics for AI agents: planetary KP-index (with G1–G5 storm classification), Schumann resonance amplitude measured at Tomsk State University (continuous since 1997), solar wind speed, and solar X-ray flux class. One JSON call, refreshed every 5 minutes. $0.05 USDC per call on Base mainnet via self-hosted facilitator (xpay), no API keys or signup. x402 v2 with Bazaar discovery extension. Free rounded preview at `/v1/preview`. ([Endpoint](https://x402.dotrockets.com/v1/cosmic)) ([Preview](https://x402.dotrockets.com/v1/preview))
